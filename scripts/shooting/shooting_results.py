...
def main():
    ...
    records = [
        ShootingResultsViz(
            ...
            event_url_qual=None if pd.isna(row.get("event_url_qual")) else row.get("event_url_qual"),
            event_url_final=None if pd.isna(row.get("event_url_final")) else row.get("event_url_final"),
