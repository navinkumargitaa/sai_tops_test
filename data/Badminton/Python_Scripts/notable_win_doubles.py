...
def determine_win_loss_notable(row: pd.Series) -> pd.Series:
    """
    Determines which team won, lost, and whether it was a notable win.
    Returns a Series with win, loss, notable_win, and lost_to fields.
    """
...
# Apply the win/loss logic for each row
...
def summarize_team_tournament(group: pd.DataFrame) -> pd.Series:
    """
    Summarizes one group of matches by selecting the best round and collecting notable wins and losses.
    """
...
# Group by team and tournament, and apply the summarization logic
tournament_summary_df = (
    all_matches_df.groupby(['team_id', 'tournament_name'])
    .apply(summarize_team_tournament)
    .dropna()
    .reset_index(drop=True)
)

tournament_summary_df['tournament_year'] = pd.to_numeric(tournament_summary_df['tournament_year'], errors='coerce')

def place_team_id_first(df: pd.DataFrame) -> pd.DataFrame:
    """
    Rearranges the DataFrame so that 'team_id' is the first column.
    """
...
# Save to CSV year-wise where `lost_to` is not empty
...
def save_notable_wins_to_db(year: int, model_cls):
    """
    Loads the CSV file for the given year and inserts the records into the corresponding database table.
    """
