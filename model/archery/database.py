...
sai_db_engine = create_engine(url="mysql+pymysql://root:root@localhost:3306/sai_archery_final")
...
def read_archery_ranking():
    ...
    FROM sai_archery_final.archery_athlete_ranking_history a
    JOIN sai_archery_final.archery_athlete b
...
def read_archery_comp_ranking():
    ...
    FROM
        sai_archery_final.archery_competition_ind_ranking a
    JOIN
        sai_archery_final.archery_competition b
        ON a.competition_id = b.competition_id
    WHERE
        a.athlete_id IN ({ID_LIST_STR})
    AND
         b.competition_sublevel NOT IN (52, 7);
    """
...
def read_archery_qual_results():
    ...
    query = f"""
            ...
            FROM (
                ...
                FROM sai_archery_final.archery_competition_individual_arrow_detail
            ) sub
            WHERE rn = 1
        )
        ...
        SELECT
            ...
            b.competition_sublevel AS sublevel_id,
            b.sub_level AS comp_sub_level,
            b.date_from AS comp_date,
            c.gold_header AS gold_header
            FROM sai_archery_final.archery_competition_ind_qual_result a
            JOIN sai_archery_final.archery_competition b
                ON a.competition_id = b.competition_id
            LEFT JOIN arrow_detail_dedup c
                ON c.competition_id = a.competition_id AND c.athlete_id = a.athlete_id
            WHERE b.competition_sublevel NOT IN (52, 7)
              AND a.athlete_id IN ({ID_LIST_STR});
    """
...
def read_archery_elem_results():
    ...
    SELECT
        ...
        b.sub_level AS comp_sub_level,
        b.date_from AS comp_date
    FROM sai_archery_final.archery_competition_individual_match_competitor a
    JOIN sai_archery_final.archery_competition b
        ON a.competition_id = b.competition_id
    LEFT JOIN sai_archery_final.archery_competition_individual_match c
